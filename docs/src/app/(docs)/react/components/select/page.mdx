...
| Key                                         | Action                                                                      |
| :------------------------------------------ | :-------------------------------------------------------------------------- |
| `Enter` / `Space` / `ArrowDown` / `ArrowUp` | Opens the popup                                                             |
| Printable character(s)                      | Selects the first item whose label starts with the typed string (typeahead) |

### Popup (open)

| Key                     | Action                                                                   |
| :---------------------- | :----------------------------------------------------------------------- |
| `ArrowDown` / `ArrowUp` | Moves highlight to next / previous item                                  |
| `Home` / `End`          | Moves highlight to first / last item                                     |
| `PageDown` / `PageUp`   | Moves highlight 10 items forward / backward, stopping at list boundaries |
| `Enter` / `Space`       | Selects the highlighted item and closes the popup                        |
| `Escape`                | Closes the popup without changing the selection                          |
| `Tab`                   | Closes the popup and moves focus to the next focusable element           |
| Printable character(s)  | Moves highlight to the next item matching the typed string (typeahead)   |
